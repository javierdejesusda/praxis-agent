...
Our approach combines five deterministic directional signal agents with ADX-based regime detection~\citep{Wilder1978}, a hierarchical risk governor enforcing hard position limits, and a walk-forward validation framework that explicitly measures overfitting degradation.

Our main contributions are as follows:
\begin{itemize}[nosep]
  \item We describe a multi-agent signal consensus architecture with five directional agents (trend, volatility, momentum, mean-reversion, and swing structure) and formalize the consensus mechanism.
...
At each cycle, the pipeline executes sequentially: (1)~data ingestion and feature computation, (2)~regime classification, (3)~parallel signal generation by five independent directional agents, (4)~signal consensus, (5)~risk governor evaluation, and (6)~execution via next-bar fill.
...
\begin{tikzpicture}[
  ...
  \node[block, right=1.2cm of data] (feat) {Feature Engine\\(25 indicators)};
...
\end{tikzpicture}
\caption{\textbf{Praxis system architecture.} Hourly data is resampled to 4-hour bars, passed through a 25-indicator feature engine, classified by ADX regime, processed by five parallel directional signal agents, aggregated via plurality-vote consensus, filtered by a hierarchical risk governor with six risk-violation kill criteria plus two signal-quality gates, and executed with next-bar-open fills including volatility-scaled slippage and exchange fees.}
...
Five directional signal agents run in parallel at each bar.
Each outputs LONG, SHORT, or NEUTRAL with a confidence score $c_i \in [0, 100]$ and a minimum threshold $\tau_i$ below which it outputs NEUTRAL.
A sixth agent (breakout signal) is defined in the codebase but is not registered in the backtester's signal dispatch table (\texttt{\_SIGNAL\_FUNCS} in \texttt{src/backtester.py}) and is excluded from the analysis.
A previous revision of this system also included a non-directional \texttt{spread\_cost} gating agent; it was removed from the backtest path in the current revision after its functionality was fully subsumed by the \texttt{SPREAD\_TOO\_WIDE} hard kill criterion in the risk governor (\S\ref{sec:risk}), and the five agents listed below are now the only ones that vote.
...
\textbf{Agent 5: Swing Structure Signal} ($\tau_5 = 30$).
...
Figure~\ref{fig:heatmap} summarizes the feature dependencies across all five agents.
...
Per-agent ablation results appear in \S\ref{sec:ablations} (Table~\ref{tab:ablation_agents}): removing \texttt{trend} alone collapses out-of-sample Sharpe from 1.239 to 0.456 ($-63.2\%$), removing \texttt{volatility} cuts it to 0.891 ($-28.1\%$), while removing \texttt{swing\_structure} has only a minor effect ($-2.7\%$).
Removing \texttt{momentum} actually \emph{improves} out-of-sample Sharpe to 1.347 ($+8.7\%$), and removing \texttt{mean\_reversion} leaves OOS essentially unchanged ($+0.4\%$).
The \texttt{trend} and \texttt{volatility} agents are therefore the clearly load-bearing members of the ensemble; we discuss the vestigial-or-worse status of \texttt{momentum} and \texttt{mean\_reversion} alongside Table~\ref{tab:ablation_agents}.
...
\begin{equation}
  ...
  \label{eq:consensus}
\end{equation}
where $\mathcal{D} = \{1, 2, 3, 4, 5\}$ is the set of directional agents, $d^*$ is the plurality direction, and $C^*$ is the aggregate confidence.
...
The aggregate confidence $C^*$ must exceed a configurable threshold (current default: $\tau_{\text{paper}} = 70$ for paper trades, $\tau_{\text{erc}} = 85$ for on-chain-validated trades) for the signal to proceed to risk evaluation; spread-based gating is applied downstream in the risk governor (\S\ref{sec:risk}) rather than as a signal-level penalty.
\label{sec:consensus}
...
The per-trade \emph{risk fraction} is determined by a half-Kelly criterion~\citep{Kelly1956, Thorp2008Kelly} with the fraction capped at 3\% and with trades rejected outright when the Kelly estimate is non-positive:
\begin{equation}
  f^* = \frac{1}{2} \cdot \frac{p \cdot b - (1 - p)}{b}, \qquad
  r =
  \begin{cases}
    \min(f^*, 0.03) & \text{if } f^* > 0 \\
    \text{reject trade (\texttt{NO\_EDGE})} & \text{otherwise}
  \end{cases}
  \label{eq:kelly}
\end{equation}
where $p \in [0,1]$ is the estimated win probability, computed as the \emph{average} per-agent confidence of aligned agents divided by 100 (not the aggregate $C^*$, which can exceed 100), blended 50/50 with the historical win rate after $\geq 10$ closed trades.
The average win/loss ratio $b$ is estimated empirically from the trailing closed trades on the current pair via the \texttt{\_estimate\_win\_loss\_ratio()} helper in \texttt{src/agents/risk\_governor.py}, which maintains a rolling window of up to 30 closed trades drawn from \texttt{Portfolio.closed\_trades} and returns the ratio of mean winning percent to mean losing percent, blended with a conservative prior of $b_0 = 1.5$ by a linear weight that grows with the number of observed trades.
When fewer than 5 qualifying trades are available for the pair (or when the window lacks either wins or losses), the helper returns the $b_0 = 1.5$ prior unchanged; once the history exceeds 10 closed trades it produces materially different per-pair ratios for BTC and ETH, which is the primary reason the numbers in Table~\ref{tab:full_history} shifted between this revision and the prior one.

The position size in USD is then:
\begin{equation}
  S = \min\!\left(\frac{r \cdot E}{m_s \cdot \text{ATR} / \text{EMA}_{21}},\; E \cdot p_{\max}\right)
  \label{eq:sizing}
\end{equation}
where $E$ is current equity, $m_s$ is the ATR stop multiplier (reference implementation: $m_s = 4.0$; see Eq.~\eqref{eq:stops}), $m_s \cdot \text{ATR}/\text{EMA}_{21}$ is the stop-distance-adjusted volatility-normalized risk factor, and $p_{\max}$ caps the \emph{position size} (not the risk fraction) as a fraction of equity.
...
Including $m_s$ in the denominator ensures that the nominal Kelly risk fraction $r$ corresponds to the actual loss incurred at the stop distance ($m_s \cdot \text{ATR}$), rather than to the raw one-ATR distance; a prior revision of the sizing formula omitted this factor and under-risked by a factor of $m_s$, which is the specific bug that caused the earlier revision's artificially low trade counts.
The separate drawdown scaling mechanism (Eq.~\eqref{eq:dd_scaling}) remains in place as a secondary safety net.
...
\textbf{Spread-based gating.} The risk governor's \texttt{SPREAD\_TOO\_WIDE} hard kill criterion (Section~\ref{sec:risk}) rejects any trade whose current bid--ask spread exceeds $20\bps$, ensuring that candidate entries always have at least ${\sim}3\times$ headroom over round-trip slippage before they reach the sizing stage.
...
Note that the fold boundaries in Table~\ref{tab:walkforward_validate} (three equal test-only spans over 2015-08 -- 2026-04) are \emph{not} the same calendar windows as the fold boundaries in Table~\ref{tab:walkforward} (train-then-test pairs); a reader comparing ``Fold~1'' across the two tables would be comparing different time ranges. We include an explicit test-span column in Table~\ref{tab:walkforward_validate} to make the difference unambiguous.
...
\textbf{Reference-config walk-forward (Table~\ref{tab:walkforward_validate}).}
The grid-optimized results above measure the \emph{optimization pipeline}, not the committed reference configuration.
When the frozen \texttt{paper-v1} config is evaluated directly via \texttt{--validate-config}, two of the three folds produce negative Sharpe ratios ($-0.83$ and $-0.61$) with single-digit trade counts (5 and 8), while only the first fold (2015-08 -- 2019-02) posts a positive Sharpe of $+0.73$ on 27 trades.
This is a stronger negative finding than the grid-optimized walk-forward above: locking the configuration in advance produces materially worse fold-by-fold performance than letting the optimizer re-tune per fold.
Two caveats apply.
First, the fold boundaries differ between Tables~\ref{tab:walkforward} and~\ref{tab:walkforward_validate} because \texttt{--validate-config} uses three equal-length test-only spans rather than train/test pairs, so ``Fold~1'' is not the same calendar window in the two tables.
Second, the 5- and 8-trade counts in the failing folds are well below the 20-trade minimum for a meaningful Sharpe estimate~\citep{lo2002statistics}, so those point estimates are better read as ``the reference config barely fires'' than as reliable risk-adjusted performance numbers.
Even after both caveats, the locked-config walk-forward weakens the claim that the specific committed \texttt{paper-v1} numbers generalize, and is the result we treat as the most damaging single finding in the paper (\S\ref{sec:conclusion}).
